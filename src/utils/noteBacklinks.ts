 ...
 * - links to `/notes/{id}` from its overview or body Markdown.
 */
...
/** Capture note slugs from Markdown hrefs and absolute site URLs. */
const NOTE_HREF_RE =
  /(?:\]\()\s*(?:https?:\/\/(?:www\.)?davidhoang\.com)?\/notes\/([a-z0-9][\w-]*)\/?(?:[?#][^)\s]*)?\s*\)/gi;
...
function linkedIdsFromNote(note: NoteBacklinkSource): Set<string> {
  const ids = new Set<string>(note.data.relatedNotes ?? []);
