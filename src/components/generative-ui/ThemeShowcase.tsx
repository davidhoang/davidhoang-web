  ...
  if (themesWithShowcase.length === 0) {
    return (
      <div style={{
        ...
        <p style={{ margin: 'var(--spacing-sm) 0 0', fontSize: '0.8rem' }}>
          Run <code style={{ background: 'var(--color-sidebar-bg)', padding: 'calc(var(--spacing-xs) - 0.1rem) 0.var(--spacing-3xl)', borderRadius: '3px' }}>npm run generate-theme</code> to create one.
        </p>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {themesWithShowcase.map((t, i) => (
            <button
              ...
              onClick={() => setIndex(i)}
              style={{
                padding: '0.var(--spacing-2xl) 0.6rem',
                fontSize: '0.75rem',
