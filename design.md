...
2. **No idle frame between hovers** — when moving between cards in a group, do not clear hover state on `mouseleave` if the pointer entered another card in the same `.cards-wrapper`. Use `handleCardHoverLeave()` from `src/components/hero/cardHover.ts` (checks `relatedTarget` / `elementFromPoint` against the wrapper) and the delayed clear from `createStableCardHoverSetter()` so Safari/iPad null `relatedTarget` does not flash rest.
