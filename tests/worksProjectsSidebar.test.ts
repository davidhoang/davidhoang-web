...
const mainLayout = readFileSync(join(process.cwd(), 'src/layouts/MainLayout.astro'), 'utf8');
...
describe('Works projects sidebar', () => {
  it('renders projects in a sidebar column beside the works stream', () => {
    ...
    expect(worksPage).toContain('<aside class="works-projects"');
    expect(worksPage).toContain('data-sticky-sidebar');
  });

  it('opts the page out of the mood filter so the sticky panel pins', () => {
    expect(worksPage).toContain('disableContentMoodFilter');
    expect(mainLayout).toContain('[data-sticky-sidebar]');
  });
